...
void m_rotate(byte* cs){
    byte temp[8] = {};
    byte btemp = 0;
    byte v = 0;
    for(byte ti = 0; ti < 8; ti++){
        btemp = 0x00;
        // ti[0] = cs[0] & 0x80 + (cs[1] & 0x80) >> 1 + (cs[2] & 0x80) >> 2 ..
        // ti[1] = cs[0] & 0x70 << 1 + (cs[1] & 0x70)  + (cs[2] & 0x70) >> 1 ..
        // ti[2] = cs[0] & 0x60 << 2 + (cs[1] & 0x70)  + (cs[2] & 0x70) >> 1 ..
        for(byte tj = 0; tj < 8; tj++){
            v = (0x80 >> ti) & cs[7-tj];
            if(tj >= ti){
                v = v >> (tj - ti);
            } else {
...
void LedMatrix::rotate(byte r){
    for(byte k = 0; k < myNumberOfDevices; k++){
        m_rotate(&cols[k*8]);
        m_rotate(&cols[k*8]);
        m_rotate(&cols[k*8]);
    }
